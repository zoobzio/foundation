...
import { commonStubs } from "#test/stubs";
import { clearError } from "#imports";

// error.vue calls clearError() from #imports — spy on it via the shim
// (vi.mock is hoisted above the imports above regardless of source order).
vi.mock("#imports", async (importOriginal) => ({
  ...(await importOriginal<typeof import("#imports")>()),
  clearError: vi.fn(),
}));

const ErrorComponent = (await import("#foundation/error.vue")).default as DefineComponent;
