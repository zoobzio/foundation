...
import Part from "#foundation/components/common/dialog/title.vue";

const factory = (props: Record<string, unknown> = {}, slots: Record<string, unknown> = {}) =>
  mount(Part, { props, slots, global: { stubs: rekaStubs("DialogTitle") } });

describe("common/dialog/Title", () => {
  it("renders with f-dialog-title class", () => {
    const wrapper = factory();
    expect(wrapper.classes()).toContain("f-dialog-title");
  });
  ...
  it("renders the aria channel", () => {
    const wrapper = factory({ aria: { level: 2 } });
